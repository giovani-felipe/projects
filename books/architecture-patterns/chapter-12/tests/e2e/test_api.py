...
@pytest.mark.usefixtures('restart_api')
def test_happy_path_returns_202_and_batch_is_allocated(client):
    orderid = random_orderid()
    sku, othersku = random_sku(), random_sku("order")

    earlybatch = random_batchref(1)
    laterbatch = random_batchref(2)
    otherbatch = random_batchref(3)

    post_to_add_batch(laterbatch, sku, 100, '2024-10-10', client)
    post_to_add_batch(earlybatch, sku, 100, '2024-10-09', client)
    post_to_add_batch(otherbatch, othersku, 100, None, client)

    response = post_to_allocate(orderid, sku, qty=3, client=client)
    assert response.status_code == 202

    response = get_allocation(orderid, client)
    assert response.ok
...
def test_unhappy_path_returns_400_and_error_message(client):
    ...
    response = post_to_allocate(orderid, unknown_sku, qty=20, expect_success=False, client=client)
